        ...
        <div className="w-[50px] md:w-[200px] pt-6 pb-10 bg-white flex flex-col border-r-[3px] border-violet-500 border-opacity-20">
            ...
            <nav>
                <ul className="flex flex-col gap-2">
                    {menu.map(item =>
                        <li key={item.href}>
                            <SidebarNavItem item={item} active={router.pathname === item.href} />
                        </li>)
                    }
                </ul>
            </nav>
...
function SidebarNavItem({ item, className, active }: { item: MenuAsideItem, className?: string, active: boolean }) {
    return (
        <Link
            href={item.href}
            className={clsx(active ? "bg-gradient-to-r from-violet-100 border-l-4 border-violet-700 text-violet-600 font-semibold" : 'text-gray-500', "flex items-center gap-2 h-[40px] md:h-[50px] px-[12px] md:px-6", className)}
        >
